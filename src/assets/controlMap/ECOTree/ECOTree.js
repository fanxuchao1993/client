...
ECOTree = function (obj, elm) {
	this.config = {
		...
		nodeSelColor : "#fff",
		levelColors : ["#fff","#fff","#fff","#fff"],
		levelBorderColors : ["#fff","#fff","#fff","#fff"],
		...
		expandedImage : '/assets/controlMap/ECOTree/img/less.gif',
		collapsedImage : '/assets/controlMap/ECOTree/img/plus.gif',
		transImage : '/assets/controlMap/ECOTree/img/trans.gif'
	}
		...
		if (!node._isAncestorCollapsed())
		{
			switch (this.render)
			{
				case "CANVAS":
					...
					if (node.canCollapse) {
						s.push('<a class="a1" id="c' + node.id + '" href="javascript:'+this.obj+'.collapseNode(\''+node.id+'\', true);" >');
					...
					{
						s.push('<a class="img01"id="t' + node.id + '" href="'+node.target+'">');
						s.push(node.dsc);
						s.push('</a>');
					}				
					else
				...
				case "VML":
					...
					s.push('style=" top:'+node.YPosition+'; left:'+node.XPosition+' !important; width:'+node.w+'; height:'+node.h+'" ');
					if (this.config.selectMode != ECOTree.SL_NONE)
						s.push('href="javascript:'+this.obj+'.selectNode(\''+node.id+'\', true);" ');										
					s.push('>');
					s.push('<font face=Verdana size=1>');
					if (node.canCollapse) {
						s.push('<a class="a1" href="javascript:'+this.obj+'.collapseNode(\''+node.id+'\', true);" >');
						s.push('<img border=0 src="'+((node.isCollapsed) ? this.config.collapsedImage : this.config.expandedImage)+'" >');							
						s.push('</a>');
						s.push('<img src="'+this.config.transImage+'" >');						
					}					
					...
					switch (this.config.nodeFill) {
						case ECOTree.NF_GRADIENT:
							s.push('<v:fill type=gradient color2="'+((node.isSelected)?this.config.nodeSelColor:color)+'" color="#fff" angle=90 />');	
							break;
						case ECOTree.NF_FLAT:
							s.push('<v:fill type="solid" color="'+((node.isSelected)?this.config.nodeSelColor:color)+'" />');	
							break;
					}
					s.push('<v:shadow type="single" on="true" opacity="0" />');					
					s.push('</v:roundrect>');																									
					break;
			}	
