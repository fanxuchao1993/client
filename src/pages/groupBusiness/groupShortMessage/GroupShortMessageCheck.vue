		    ...
		    >
			    ...
			    <el-table-column prop="ERRORA" label="A单边(CRM)" width='110px'>
			    	<template slot-scope="scope">
			        	<el-button @click="handleClick(scope.row)" type="text" size="small">{{scope.row.ERRORA}}</el-button>
			      	</template>	
			    </el-table-column>
			    <el-table-column prop="ERRORB" label="B单边(平台)" width='100px'>
			    	<template slot-scope="scope">
			        	<el-button @click="handleClick(scope.row)" type="text" size="small">{{scope.row.ERRORB}}</el-button>
			      	</template>	
			    </el-table-column>
			    ...
			    <el-table-column prop="CONSIS_NUM" label="不一致条数" width='100px'></el-table-column>
			    ...
			    <el-table-column prop="CONSIS_RATIO" label="关联失败订单数"></el-table-column>
			    <el-table-column prop="CONSIS_RATIO" label="关联失败订单率(%)"></el-table-column>
	...
	export default{
		name: 'GroupShortMessageCheck',
	  	...
	  	methods: {
			...
			handleClick(row){
				this.$router.push({name:"GroupShortMessageCheckABN"});
			},
