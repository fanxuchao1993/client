	...
	<div>
		...
		<div class="pageLocal">
			<el-breadcrumb separator-class="el-icon-arrow-right">
			  	...
			  	<el-breadcrumb-item>234G协同修复</el-breadcrumb-item>
			</el-breadcrumb>
		</div>
		<!--数据表格-->
		<div class="tableMenu">
			<div class="content-top">
				<div class="divStyle">
					<i class="iconfont icon-biaotishutiao6x25"></i>234G协同修复总览
				</div>
				<div class="">
		    ...
		    >
			    ...
			    <el-table-column label="操作" fixed="right">
			    	<template slot-scope="scope">
			    		<el-button @click="handleQuery(scope.row)" type="text" size="small">[查询]</el-button>
	      		...
	      		getResultRepair(
					{
			    		...
			    		offerId:row.OFFER_ID,
			    		spCode:row.SP_CODE,
			    		operatorCodes:row.OPERATOR_CODE
					}
				).then(res=>{
					console.log('请求成功！'+JSON.stringify(res.data.response));
					this.gridData = res.data.response.reList;
					this.dialogTableVisible = true;
					this.diaPhone = row.BILL_ID;
				}).catch(err=>{
					console.log(err);
					console.log('请求失败！');
				})
